...
from metrics.pedestrian_metrics import get_pedestrian_metrics, save_label_wise_metrics_to_csv
...
def trainer(cfg, args, epoch, model, model_ema, train_loader, valid_loader, criterion, optimizer, lr_scheduler,
    ...
    for e in range(epoch):
        ...
        if cfg.METRIC.TYPE == 'pedestrian':
            #calculate metrics label-wise and instance level
            train_result = get_pedestrian_metrics(gt_label= train_gt, 
                                                  preds_probs= train_probs, 
                                                  attr_names= attr_names, 
                                                  index=None, 
                                                  cfg=cfg)
            valid_result = get_pedestrian_metrics(gt_label= valid_gt, 
                                                  preds_probs= valid_probs, 
                                                  attr_names= attr_names, 
                                                  index=None, 
                                                  cfg=cfg)
            #save label-wise metrics to csv in each epoch
            save_label_wise_metrics_to_csv(train_result,
                                            cfg=cfg,
                                            epoch=e,
                                            train=True,
                                            seed = args.seed)
            save_label_wise_metrics_to_csv(valid_result,
                                            cfg=cfg,
                                            epoch=e,
                                            seed = args.seed)
